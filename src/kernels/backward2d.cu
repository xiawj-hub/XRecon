...
__global__ void fanflat_backward_cuda_kernel(
    ...
    for (int idx_view = 0; idx_view < num_view; idx_view++) {
        ...
        while (bound0 < bound1) {
            ...
            float d1 = useXAxis ? map2x(src, det_next)
                                : map2y(src, det_next);
            d1 = fminf(d1, bound1);
            float p = tex3D<float>(tex_obj, idxd, idx_view, idx_batch) * geo.du;
            acc += (d1 - bound0) * p;
            bound0 = d1;
            det_cur = det_next;
            idxd += keepDim ? 1 : -1;
    ...
    img[(idx_batch * vol.Ny + (vol.Ny - 1 - idx_row)) * vol.Nx + idx_col] = pix_val;
}

template<bool FBPWEIGHT>
__global__ void fanarc_backward_cuda_kernel(
    cudaTextureObject_t tex_obj,
    float* __restrict__ img,
    const float* __restrict__ ang,
    const Volume2d vol,
    ...
    for (int idx_view = 0; idx_view < num_view; idx_view++) {
        ...
        while (bound0 < bound1) {
            ...
            float d1 = useXAxis ? map2x(src, det_next_ang)
                                : map2y(src, det_next_ang);
            d1 = fminf(d1, bound1);
            float p = tex3D<float>(tex_obj, idxd, idx_view, idx_batch) * det_rad;
            acc += (d1 - bound0) * p;
            bound0 = d1;
...
__global__ void parallel_backward_cuda_kernel(
    cudaTextureObject_t tex_obj,
    float* __restrict__ img,
    const float* __restrict__ ang,
    const Volume2d vol,
    ...
    for (int idx_view = 0; idx_view < num_view; idx_view++) {
        ...
        float det_interval = useXAxis ? (geo.du / fabsf(cosv))
                                      : (geo.du / fabsf(sinv));
        ...
        int idxd = floorf((bound0 - (useXAxis ? d0p : d0p)) / det_interval);
        idxd = max(0, idxd);
        idxd = min(geo.Nu - 1, idxd);

        float det0p = (useXAxis ? d0p : d0p) + idxd * det_interval; 
        idxd = keepDim ? idxd : (geo.Nu - 1 - idxd);

        while (bound0 < bound1) {
            float d1 = det0p + det_interval;
            d1 = fminf(d1, bound1);
            float p = tex3D<float>(tex_obj, idxd, idx_view, idx_batch) * coef;
            pix_val += (d1 - bound0) * p;
            bound0 = d1;
            det0p  = d1;
            idxd += keepDim ? 1 : -1;
...
void backward2d_cuda(
    ...
    texture3D.create(proj, geo.Nu, num_view, num_batch);
    
    dim3 blockDim(BLOCK_DIM, BLOCK_DIM);
    int block_y = (vol.Ny - 1) / BLOCK_DIM + 1;
    int block_z = (vol.Nx - 1) / BLOCK_DIM + 1;
    dim3 gridDim(num_batch, block_y, block_z);

    switch (geo.type) {
    case GeometryType::FanFlat:
        fanflat_backward_cuda_kernel<false><<<gridDim, blockDim>>>(
            texture3D.texObj, image, ang, vol, geo, num_view);
        break;
    case GeometryType::FanArc:
        fanarc_backward_cuda_kernel<false><<<gridDim, blockDim>>>(
            texture3D.texObj, image, ang, vol, geo, num_view);
        break;
    case GeometryType::Parallel:
        parallel_backward_cuda_kernel<<<gridDim, blockDim>>>(
            texture3D.texObj, image, ang, vol, geo, num_view);
        break;
    }
...
void weighted_backward2d_cuda(
    ...
    switch (geo.type) {
    case GeometryType::FanFlat:
        fanflat_backward_cuda_kernel<true><<<gridDim, blockDim>>>(
            texture3D.texObj, image, ang, vol, geo, num_view);
        break;
    case GeometryType::FanArc:
        fanarc_backward_cuda_kernel<true><<<gridDim, blockDim>>>(
            texture3D.texObj, image, ang, vol, geo, num_view);
        break;
    case GeometryType::Parallel:
        parallel_backward_cuda_kernel<<<gridDim, blockDim>>>(
