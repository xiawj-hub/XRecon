...
__global__ void conearc_forward_cuda_kernel(
...
__global__ void conearc_forward_t_cuda_kernel(
...
void forward3d_arc_cuda(
    ...
    conearc_forward_cuda_kernel<<<grid_dim, BLOCK_DIM>>>(
        proj, texture3D.texObj, ang, vol, geo, num_view);
}
void forward3d_t_arc_cuda(
    float *image,
    float *proj,
    float *ang,
    Volume3d vol,
    ...
    conearc_forward_t_cuda_kernel<<<grid_dim, BLOCK_DIM>>>(
